...
def run_yolo():
    ...
    retry_count = 0
    cap = None
    while not stop_event.is_set() and retry_count < 10:
        cap = cv2.VideoCapture(RTSP_STREAM_URL)
        if cap.isOpened():
            print(f"Connected to RTSP stream: {RTSP_STREAM_URL}")
            break
        retry_count += 1
        print(f"Retrying RTSP connection... ({retry_count})")
        time.sleep(5)

    if not cap or not cap.isOpened():
        print("Failed to open RTSP stream after retries. Exiting thread.")
        return
