...
class BallTracker:
    def __init__(self, cone_distance_ft, mph_conversion, fps=30):
        self.cone1_line = None
        self.cone2_line = None
        self.armed = False
    ...
    def set_static_cones(self, cone1_start, cone1_end, cone2_start, cone2_end, frame_width, frame_height):
        self.cone1_line = (cone1_start, cone1_end)
        self.cone2_line = (cone2_start, cone2_end)
        self.frame_width = frame_width
        self.frame_height = frame_height
        print(f"[STATIC CONES] Cone 1 line: {cone1_start} → {cone1_end}")
        print(f"[STATIC CONES] Cone 2 line: {cone2_start} → {cone2_end}")

    def _is_past_line(self, point, line_start, line_end):
        x, y = point
        return (line_end[0] - line_start[0]) * (y - line_start[1]) - \
               (line_end[1] - line_start[1]) * (x - line_start[0]) < 0

    def update(self, center_x, center_y, current_frame):
        status = "Waiting for ball"
        result = None

        if not self.cone1_line or not self.cone2_line:
            return status, None

        if not self.armed and not self._is_past_line((center_x, center_y), *self.cone1_line):
            self.armed = True
            self.cross_registered = False
            self.launch_point = (center_x, center_y)
            print(f"[ARMED] Ball seen before Cone 1 at ({center_x}, {center_y})")
            status = "Armed"

        if self.armed and not self.cross_registered and self._is_past_line((center_x, center_y), *self.cone1_line):
            self.cross_frame_1 = current_frame
            self.cross_registered = True
            print(f"[CONE 1 CROSSED] at ({center_x}, {center_y}) (frame {current_frame})")
            status = "Crossed Cone 1"

        if self.armed and self.cross_frame_1 is not None and self._is_past_line((center_x, center_y), *self.cone2_line):
            cross_frame_2 = current_frame
            elapsed = (cross_frame_2 - self.cross_frame_1) / self.fps
            print(f"[CONE 2 CROSSED] at ({center_x}, {center_y}) (frame {current_frame}), elapsed={elapsed:.3f}s")
